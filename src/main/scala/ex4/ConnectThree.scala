...
object ConnectThree extends App:
  ...
  def computeAnyGame(player: Player, moves: Int): LazyList[Game] = (player, moves) match
    case (p, 0) => LazyList(List(List()))
    case (p, m) =>
      for
        game <- computeAnyGame(p.other, m - 1)
        move <- placeAnyDisk(game.last, p)
      yield game :+ move
