...
trait NotTwoConsecutive[T] extends Parser[T]:
  private[this] var lastToken: Option[T] = Option.empty
  private[this] var valid: Boolean = true

  abstract override def parse(t: T): Boolean =
    if this.lastToken.getOrElse(None).equals(t) then
      this.valid = false
      false
    else
      this.lastToken = Option(t)
      super.parse(t)

  abstract override def end: Boolean = this.valid && super.end

class NotTwoConsecutiveParser(chars: Set[Char])
    extends BasicParser(chars)
    with NotTwoConsecutive[Char]

trait ShorterThanN[T](val n: Int) extends Parser[T]:
  private[this] var length: Int = 0

  abstract override def parse(t: T): Boolean =
    this.length += 1
    if this.length > n then false else super.parse(t)

  abstract override def end: Boolean = this.length <= n && super.end
